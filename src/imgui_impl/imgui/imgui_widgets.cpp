...
void ImGui::Image(ImTextureID user_texture_id, ImVec2 size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col, bool preserve_aspect_ratio)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
	...
	{
		float titlebar_size = GImGui->FontSize + GImGui->Style.FramePadding.y * 3;
		float x_padding = GImGui->Style.FramePadding.x * 4;
		float y_padding = titlebar_size + GImGui->Style.FramePadding.y * 4;

		ImVec2 available_size = window->Size - ImVec2(x_padding, y_padding);

		float scale_down_multiplier = 1.f;
		if (size.x > available_size.x)
		{
			scale_down_multiplier = available_size.x / size.x;
		}
		if (size.y > available_size.y)
		{
			scale_down_multiplier = glm::min(scale_down_multiplier, available_size.y / size.y);
		}

		size *= scale_down_multiplier;

		position = (available_size - size) / 2;
	}

    ImRect bb(window->DC.CursorPos + position, window->DC.CursorPos + position + size);
