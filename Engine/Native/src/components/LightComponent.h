...
namespace plumbus
{
	enum class LightType
    	...
    	{
    	public:
    		Light(glm::vec3 colour, components::LightComponent* parent) : m_Colour(colour), m_Shadow(nullptr), m_LightComponent(parent) {}
    		virtual ~Light() {}
    		...
    		components::LightComponent* GetParent() { return m_LightComponent; }
    		void RemoveShadow() { if (m_Shadow) { m_Shadow.reset(); } }
    	protected:
    		glm::vec3 m_Colour;
    		LightType m_Type = LightType::Invalid;
    		vk::ShadowRef m_Shadow;
    		components::LightComponent* m_LightComponent;
    	};
    
    	class DirectionalLight : public Light 
    	{
    	public:
    		DirectionalLight(glm::vec3 colour, glm::vec3 direction, components::LightComponent* parent) : Light(colour, parent), m_Direction(direction) { m_Type = LightType::Directional; }
    		~DirectionalLight() {}
    	...
    	{
    	public:
    		PointLight(glm::vec3 colour, float radius, components::LightComponent* parent) : Light(colour, parent), m_Radius(radius) { m_Type = LightType::Point; }
    		~PointLight() {}
